...
上方三個面板固定顯示 build / serve / process 狀態，中間可切換五個檢視：

| 鍵  | 檢視   | 內容                                                         |
| --- | ------ | ------------------------------------------------------------ |
| `1` | 記憶體 | 私有／工作集／受控堆疊／Gen2／LOH／CPU 走勢，GC 次數，成長量 |
| `2` | 堆疊   | 型別直方圖、與基準快照的差異、根參考鏈                       |
| `3` | 執行緒 | 受控執行緒清單與呼叫堆疊（● 標記自己的程式碼）               |
| `4` | 記錄   | 設定解析結果、建置與伺服器輸出                               |
| `5` | 輸出   | 應用程式的 `Debug.WriteLine` / `Trace.WriteLine`（OutputDebugString） |
...
沒有偵錯器附加時原本會直接消失，現在會被獨立收進按鍵 `5` 的輸出檢視。

- **`5`** 切到輸出檢視；**`g`** 切換「只看附加的 PID」／「全部行程（多一欄 PID）」，
